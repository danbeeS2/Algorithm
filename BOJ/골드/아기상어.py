...
def bfs():
    ...
    visited[now_x][now_y] = 1               # 방문 체크

    while Q:
        i, j = Q.popleft()
        for di, dj in direction:
            ni, nj = i + di, j + dj
            if ni < 0 or ni >= N or nj < 0 or nj >= N: continue           # 유효하지 않은 범위이면 pass
            if visited[ni][nj] == 0 and arr[ni][nj] <= shark_size:         # 미방문 + 나갈 수 있으면
...
def find_fish(visited):
    ...
    for i in range(N):
        for j in range(N):
            if visited[i][j] != 0 and 1 <= arr[i][j] < shark_size:    # bfs로 방문했고 먹을 수 있는 물고기이면
                if min_distance > visited[i][j]:
